...
bool RayTracer::Trace(const Ray& ray, TraceInfo& trace_info)
{
	...
	static constexpr float STEP_VALUE = 0.001f;

	for (float current_dist = 0.f; current_dist <= ray.m_length; current_dist += STEP_VALUE)
	{
		const glm::vec3 current_pos(ray.m_start + ray.m_direction * current_dist);
		const glm::vec3 block_pos(std::floor(current_pos.x), std::floor(current_pos.y), std::floor(current_pos.z));

		if (Block block; !m_world->GetBlock(block_pos.x, block_pos.y, block_pos.z, block) || !block.m_is_active)
			continue;

		AABB aabb(block_pos, block_pos + 1.f);
		glm::vec3 int_normal;
		float int_dist;
	...
	trace_info.m_normal = { 0.f, 0.f, 0.f };
	trace_info.m_trace_start = ray.m_start;
	trace_info.m_trace_end = ray.m_start + ray.m_direction * ray.m_length;
	trace_info.m_fraction = 1.f;
	return false;
}
bool RayTracer::RayPlaneIntersection(const Ray& ray, const Plane& plane, float* intersection_distance)
{
	const float denom = glm::dot(plane.m_normal, ray.m_direction);
	if (std::abs(denom) <= 0.f)
		return false;

	const float distance = glm::dot(plane.m_point - ray.m_start, plane.m_normal) / denom;
	if (distance < 0.f || distance > ray.m_length)
		return false;
...
bool RayTracer::RayAABBIntersection(const Ray& ray, const AABB& aabb, float* intersection_distance, glm::vec3* intersection_normal)
{
	const auto aabb_center = (aabb.m_min + aabb.m_max) * 0.5f;
	
	glm::vec3 normal;
	float closest_distance = FLT_MAX;
	for (int i = 0; i < 3; i++)
	{
		Plane plane_1(aabb_center, { 0.f, 0.f, 0.f });
		plane_1.m_point[i] = aabb.m_max[i];
		plane_1.m_normal[i] = 1.f;

		Plane plane_2(aabb_center, { 0.f, 0.f, 0.f });
		plane_2.m_point[i] = aabb.m_min[i];
		plane_2.m_normal[i] = -1.f;
		
		if (float distance; RayPlaneIntersection(ray, plane_1, &distance) && distance <= closest_distance)
		{
			const auto int_point = ray.m_start + ray.m_direction * distance;
			if (std::abs(int_point.x - aabb_center.x) <= (aabb.m_max.x - aabb.m_min.x) * 0.5f &&
				std::abs(int_point.y - aabb_center.y) <= (aabb.m_max.y - aabb.m_min.y) * 0.5f &&
				std::abs(int_point.z - aabb_center.z) <= (aabb.m_max.z - aabb.m_min.z) * 0.5f)
			{
		...
		if (float distance; RayPlaneIntersection(ray, plane_2, &distance) && distance <= closest_distance)
		{
			const auto int_point = ray.m_start + ray.m_direction * distance;
			if (std::abs(int_point.x - aabb_center.x) <= (aabb.m_max.x - aabb.m_min.x) * 0.5f &&
				std::abs(int_point.y - aabb_center.y) <= (aabb.m_max.y - aabb.m_min.y) * 0.5f &&
